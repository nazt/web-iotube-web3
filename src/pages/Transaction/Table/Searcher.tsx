...
export const Searcher = observer((props: TableProps) => {
  ...
  return (
    <>
      {(!!(actionList.sender.value.concat(actionList.recipient.value).concat(actionList.token.value)) || actionList.status.value !== 100) &&
      <HStack mt={4} p={4}>
        <Text>Filter by: </Text>
        ...
        <Tag
          ...
          colorScheme='gray'
        >
          <TagLabel>From: <Text display={'inline'}
                                color={textColor}>{helper.string.truncate(actionList.sender.value, 12, '...')}</Text></TagLabel>
        ...
        <Tag
          ...
          colorScheme='gray'
        >
          <TagLabel>To: <Text display={'inline'}
                              color={textColor}>{helper.string.truncate(actionList.recipient.value, 12, '...')}</Text></TagLabel>
          <TagCloseButton onClick={() => record.clearSearchParam('recipient')} />
        </Tag>
        }
        {actionList.status.value != 100 &&
            <Tag
              size='md'
              borderRadius='full'
              variant='subtle'
              colorScheme='gray'
            >
              <TagLabel color={record.statusMap[actionList.status.value].color}>{record.statusMap[actionList.status.value].name}</TagLabel>
              <TagCloseButton onClick={() => record.clearSearchParam('status')} />
            </Tag>
        }
        ...
        <Tag
          ...
          colorScheme='gray'
        >
          <TagLabel>Token: <Text display={'inline'}
                                 color={textColor}>{helper.string.truncate(actionList.token.value, 12, '...')}</Text></TagLabel>
              ...
              <PopoverBody _focusVisible={{}} _focus={{}} align='right'>
                ...
                <List spacing={5} padding={4} maxH='500px' overflowY='scroll'>
                  {store.tokens.map((i, item) => (
                    <ListItem key={`${record.activeTab.value}-${item}`} cursor='pointer' display='flex'
                              alignItems='center' justifyContent='space-between' onClick={() => {
                      store.onSelect(i.address);
                      onClose();
                    }}>
...
export const SearchStatusPopover = observer((props: SearchFiledProps) => {
  const { record } = useStore();

  const store = useLocalStore(() => ({
    isOpen: new BooleanState(),
    onSelect(statuId) {
      record.updateSearchParam(props.name, statuId);
    }
  }));
  ...
  return (
    <Popover trigger={'click'}>
      {({ isOpen, onClose }) => (<>
          ...
          <PopoverContent w='180px' _focusVisible={{}} _focus={{}} p={1} borderRadius='20px'>
            <PopoverBody _focusVisible={{}} _focus={{}} align='right'>
              <List spacing={4} padding={2}>
                {Object.values(record.statusMap).map((i, item) => (
                  <ListItem key={`status-${item}`} cursor='pointer' display='flex'
                            alignItems='center' justifyContent='space-between' onClick={() => {
                    store.onSelect(i.id);
                    onClose();
                  }}>
